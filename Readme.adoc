...
We'll use the automation tool Ansible to remotely provision our Gateway system by installing required components. All you need on the host system is to install Ansible [e.g. yum install ansible].

 * Open a terminal and enter the following commands:
  
  [user@localhost ~]$ git clone -b Virtual-Lab-1 https://github.com/IshuVerma/Virtual-IoT_Gateway.git
 ...
 * Launch the Ansible playbook by entering the following commands:
  ...
  [user@localhost ~]$ cd Virtual-IoT-Gateway/Ansible
